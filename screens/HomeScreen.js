...
export const ELYSIUM_GARDENS_DETAILS = {
  ...
  videos: [
    {
      type: 'video',
      url: 'https://mock.video/elysium-tour.mp4',
      thumbnail:
        'https://images.unsplash.com/photo-1582268611958-abf29d400260?w=500&h=300&fit=crop',
    },
  ],
  facilities: [
  ...
  description:
    'The premier luxury experience in Paris. Indulge in our exquisite gardens and world-class services.',
};

export const AZURE_HEIGHTS_DETAILS = {
  id: '2',
  ...
  videos: [
    {
      type: 'video',
      url: 'https://mock.video/azure-pool.mp4',
      thumbnail:
        'https://images.unsplash.com/photo-1574977465660-f1c6d2673d32?w=500&h=300&fit=crop',
    },
    {
      type: 'video',
      url: 'https://mock.video/azure-view.mp4',
      thumbnail:
        'https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?w=500&h=300&fit=crop',
    },
  ],
  ...
  description:
    'A breathtaking view of the Italian skyline. A blend of ancient history and modern luxury.',
};

export const SOLARA_SPRINGS_DETAILS = {
  id: '3',
  ...
  rating: '4.2',
  reviewCount: '5,10',
  images: ['https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=500&h=300&fit=crop'],
  videos: [],
  facilities: [
  ...
  description:
    'Cozy and contemporary stay in the heart of London, close to all major attractions.',
};
...
const DotIndicator = ({ isActive }) => <View style={[styles.dot, isActive && styles.activeDot]} />;

const PopularHotelCard = ({ item, onPress }) => {
  const [isFavorite, setIsFavorite] = useState(false);
  return (
    <TouchableOpacity style={styles.hotelCard} onPress={() => onPress(item.id)} activeOpacity={0.8}>
        ...
        <View style={styles.hotelFooter}>
          ...
          <TouchableOpacity onPress={() => setIsFavorite(!isFavorite)}>
            <Ionicons
              ...
              color={isFavorite ? ACCENT_COLOR : TEXT_GRAY}
            />
          </TouchableOpacity>
        </View>
      </View>
...
export const HotelDetailsScreen = ({ hotel, onBackPress, onBookPress }) => {
    ...
    <View style={styles.detailsFullContainer}>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.mapContainer}>
            <Image
              source={{
                uri:
                  'https://images.unsplash.com/photo-1541818165980-007cc7282b0f?w=500&h=300&fit=crop',
              }}
      ...
      <SafeAreaView style={styles.bottomBar} edges={['bottom']}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (typeof onBookPress === 'function') {
              onBookPress();
            } else {
              Linking.openURL('http://example.com/book');
            }
          }}
          activeOpacity={0.8}
        >
...
export default function HomeScreen({ route, setDetailsMode, navigation }) {
  const useExternalDetails = typeof setDetailsMode === 'function';
  const [localViewState, setLocalViewState] = useState({ isDetails: false, details: null });

  const handleHotelPress = (hotelId) => {
    const hotelDetails = HOTEL_DETAILS_MAP[hotelId];
    if (useExternalDetails) {
      setDetailsMode({ isDetails: true, details: hotelDetails });
    } else {
      setLocalViewState({ isDetails: true, details: hotelDetails });
    }
  };

  const handleBackPress = () => {
    if (useExternalDetails) {
      setDetailsMode({ isDetails: false, details: null });
    } else {
      setLocalViewState({ isDetails: false, details: null });
    }
  };

  if (!useExternalDetails && localViewState.isDetails && localViewState.details) {
    const hotel = localViewState.details;
    return (
      <HotelDetailsScreen
        hotel={hotel}
      ...
      <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          <View style={styles.searchModule}>
            ...
            <View style={styles.inputGrid}>
              <TouchableOpacity style={styles.inputField}>
                <Ionicons name="calendar-outline" size={responsiveFont(4.5)} color={TEXT_GRAY} />
              ...
              <TouchableOpacity style={styles.inputField}>
                <Ionicons name="person-outline" size={responsiveFont(4.5)} color={TEXT_GRAY} />
                <Text style={styles.inputText}>1 Guest</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.inputField}>
                <Ionicons name="home-outline" size={responsiveFont(4.5)} color={TEXT_GRAY} />
          ...
          <View style={styles.section}>
            ...
            <FlatList
              data={popularHotels}
              renderItem={({ item }) => <PopularHotelCard item={item} onPress={handleHotelPress} />}
              keyExtractor={(item) => item.id}
              horizontal
              showsHorizontalScrollIndicator={false}
...
const styles = StyleSheet.create({
  ...
  horizontalList: { paddingHorizontal: responsiveWidth(5), bottom: responsiveHeight(0.5), height: responsiveHeight(32) + responsiveHeight(5) },
  hotelCard: {
